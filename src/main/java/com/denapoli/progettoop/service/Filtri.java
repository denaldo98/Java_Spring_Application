...
public class Filtri {
     ...
     * @return boolean
     */
    private static boolean check(Object val, String oper, Object rif) {
        if (operatori.contains(oper)) { //controllo che l'operatore sia uno di quelli gestiti
            ...
            } else if (val instanceof String || val instanceof Character) {// caso in cui il valore da controllare sia una stringa o un carattere (per freq)
               if(val instanceof Character)
                val=String.valueOf(val); //nel caso di carattere, lo riporto a stringa
                ...
                if (rif instanceof String) {        // caso in cui il riferimento sia una stringa
                    ...
                    throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Riferimento:'" + rif + "' non compatibile con il valore'" + val + "'"); //caso in cui valore e riferimento non siano compatibili
            } else
